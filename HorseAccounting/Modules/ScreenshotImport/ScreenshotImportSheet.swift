...
//                    .padding() // 图片与sheet上边别接触
            ...
            .navigationBarItems(
                ...
                HStack {
                    ...
                    }) { Text("取消").bold().foregroundColor(Color(hex:0x093C89)) }
                    ...
                    if weChatBills.count != 0 {
                        ...
                        }) { Text("清除").bold().foregroundColor(Color(hex:0x093C89)) }
                    } else {
                        EmptyView()
                    }
                },
                ...
                HStack {
                    if weChatBills.count != 0 {
                        Button(action: {
                            ...
                            RacoonSheetConfig.shared.showingScreenshotImportSheet = false
                            ClearImportSheetState()
                        }) { Text("导入").bold().foregroundColor(Color(hex:0x093C89)) }
                    } else {
                        EmptyView()
                    }
                }
...
//            RacoonSheetConfig.shared.showingSuccessfullyAlert = true // 注意不能直接在这里用这个，因为alert的内容没有指定为多条
    ...
    private func recognizeTextHandler(request: VNRequest, error _: Error?) {
        ...
        while i < recognizedStrings.count - 2 {
            let string1: String = recognizedStrings[i]      //事件
            let string2: String = recognizedStrings[i + 1]  //时间
            let string3: String = recognizedStrings[i + 2]  //开销

            let dateFormatter = DateFormatter()
            dateFormatter.dateFormat = "M月d日 HH:mm"

            if var spentAt = dateFormatter.date(from: string2),let amount = Float(string3)
            {
